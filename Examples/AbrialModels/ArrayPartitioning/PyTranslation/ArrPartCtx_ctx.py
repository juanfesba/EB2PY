...
#Translation of Context: ArrPartCtx


class ArrPartCtx_class():
   ...
   def ax0_axiomCheck(self) -> bool:
      return P.NAT().PyContains(self.x)
   ...
   def checkedInit(self, m_userIn : int = P.NoParam(), x_userIn : int = P.NoParam() ) -> None:

      if m_userIn is None:
         m_userIn = P.PyRandValGen("int")
      if x_userIn is None:
         x_userIn = P.PyRandValGen("int")
   ...
   def __str__(self) -> str:
      ...
      #Print Constants
      tmp_values.append("###")
      tmp_values.append("ArrPartCtx Constants")
